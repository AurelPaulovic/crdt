...
class MPNSet[T] private (val id: Id, private[this] val replica: Replica, private val clock: components.GCounter[Long], private val elements: immutable.Map[T, PNCounter[Int]]) {
...
object MPNSet {
  def apply[T](id: Id, replica: Replica): MPNSet[T] = new MPNSet[T](id, replica, components.GCounter[Long](replica), immutable.Map.empty)
  ...
  def apply[T](id: Id, replica: Replica, elements: T*) = {
    ...
    new MPNSet[T](id, replica,  components.GCounter[Long](replica).increment(elementPairs.size.toLong), elementPairs.toMap)
  }
}